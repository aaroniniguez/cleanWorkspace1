...
def verifyCredentials():
    cmd = "java -Xmx2048m -jar "+ cli_jar + " -s "+ server+" login"+authentication
    print "Logging in to the Jenkins server ", server
    pipe = subprocess.Popen(cmd,stdout=subprocess.PIPE,stderr=subprocess.PIPE, shell=True)
    out, err = pipe.communicate()
    if err:
        print err
    pipe.wait()
    pattern1 = re.compile(".*Failed to connect.*")
    match1= pattern1.match(err)
    if match1 :
        print "***Error: Failed to connect to jenkins server :", server, "\n Please check the URL"  
        print "\n**********************************************************\n"
        sys.exit(1)
    else:
        print"Log in was successful !!\n"
def deleteClientSpec():
    cmd = "ls"
    #
    cmd = "export P4PORT=perforce.spirentcom.com:1666;export P4USER=scmbuild;for i in `p4 clients | grep 4.63_rel | grep release_il_ | grep -v _il_ci_| awk '{print $2}'`; do p4 -u p4admin -P SpirentSCM client -d -f $i;done"
...
def cleanUpWorkspace(node):
    ...
    command = "cd "+getRootDir(node)
    cmd = "sshpass -p spirent ssh -o StrictHostKeyChecking=no "+node+" \""+command+";"+"ls\""
    pipe = subprocess.Popen(cmd,stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE,shell=True)
...
def main():
    ...
    for key,value in vars(options).iteritems():
        print key, ":", value
    print"************************************\n"
    if options.server_link == None or options.cli_jar == None:
        print "Please enter arguments correctly: The Jenkins server link, cli_jar location"
        print "Example:\npython "+curFileName+" -s http://jenkins-rel.cal.ci.spirentcom.com:8080/ -c /home/ProjectJenkins/ -v \"release(4.68_rel)\""
        sys.exit(1)
